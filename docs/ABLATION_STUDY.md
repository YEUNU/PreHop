...
Ablation 1–3은 같은 4B 통제 색인에서 질의 단계만 비교한다. Ablation 4와
강건성 실험은 이 설정에서 검색한 후보를 고정하고 점수 계산과 제시 순서를
각각 바꾼다. 결과는 표에 적힌 단계, 변경 조건, 모델 설정의 범위에서만
해석한다.
...
- **실험 방식.** 4B 통제 색인의 저장 연결을 MuSiQue 정답 문단과 전수
  대조한다.
...
- **보조 지표.** Answer F1, 문단 Support F1/Recall, 저장된 정답 연결
  유무에 따른 사후 하위집단.
- **비교 조건.** 같은 4B 통제 색인에서 깊이 1의 그래프 확장을 사용한
  실행과 깊이 0 통제.
