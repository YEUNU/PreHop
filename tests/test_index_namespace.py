...
def test_index_namespace_defaults_to_corpus_tag(monkeypatch):
    ...
    assert index_namespace("multi-hop.rag").startswith("multi_hop_rag_")
    assert index_namespace("multi-hop.rag") != index_namespace("multi_hop_rag")
...
def test_index_namespace_is_explicitly_run_scoped(monkeypatch):
    ...
    assert index_namespace("multihoprag").startswith("multihoprag_paper_run_01_")
