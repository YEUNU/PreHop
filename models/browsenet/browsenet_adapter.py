...
class BrowseNetAdapter:
    ...
    async def run_workflow(self, query: str, history: list[dict] | None = None) -> tuple[str, list, list]:
        ...
        if not documents:
            return mark_answer_boundary("Insufficient evidence."), [], [{"step": "browsenet_retrieval", "output": "empty"}]
        ...
        if not str(answer or "").strip():
            raise ValueError("BrowseNet answer synthesis returned an empty response")
        sources = [
            {
        ...
        return mark_answer_boundary(answer), sources, [
            {"step": "browsenet_official_retrieval", "retrieved": len(documents), "answer_context": len(context_documents)}
        ]
