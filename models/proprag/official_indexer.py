...
async def run_official_index(dataset_path: str, corpus_tag: str, corpus_manifest: dict | None) -> dict[str, float]:
    ...
    records, target = stage_corpus("proprag", dataset_path, corpus_tag)
    staging_seconds = time.perf_counter() - started
    worker_started = time.perf_counter()
    result = run_index_worker("proprag", corpus_tag, {"operation": "index"})
    official_seconds = time.perf_counter() - worker_started
    source_ids = [row["source_id"] for row in records]
    _write_json(
        snapshot_metadata_path("proprag", corpus_tag),
        {
            "snapshot_version": 1,
            "status": "complete",
            "strategy": "proprag",
            ...
            "retrieval_artifact_dir": str(target / "outputs"),
