"""τ_hop / τ_r sensitivity sweep (calibration story for CLAUDE.md "Reranking" —
both thresholds were tuned for the old cross-encoder classifier's 0-1
probability scores and now gate raw bi-encoder cosine similarity instead).

Two independent parameters, two different mechanics:

- `--param hop` sweeps `RAG_HOP_THRESHOLD` (τ_hop), which gates offline
...
- `--param rerank` sweeps `RERANKER_THRESHOLD` (τ_r), a pure query-time gate
  (rerank.py/traversal.py) — no graph rebuild needed at all, just repeated
  benchmark passes over the same already-indexed corpus.
...
Usage:
  python scripts/threshold_sweep.py --param hop \
  ...
  python scripts/threshold_sweep.py --param rerank \
    --values 0.3,0.4,0.5,0.6 --corpus-tag multihoprag \
    --queries_file data/multihoprag_sample200_queries.json \
    --model gemma-4-31b-it --out-dir data/results/rerank_sweep

Outputs: <out-dir>/<param>_sweep.json (full) + <out-dir>/<param>_sweep.csv
...
avg_traversal_ms, avg_synthesis_ms — *hop-only fields blank for --param rerank).
"""
...
RERANK_ENV = "RERANKER_THRESHOLD"

QUALITY_KEYS = [
    "avg_llm_judge_score", "avg_hallucination", "avg_mrr@10", "avg_map@10",
    "avg_hits@4", "avg_hits@10", "avg_evidence_doc_recall", "avg_doc_match",
    "avg_latency", "avg_retrieve_ms", "avg_traversal_ms", "avg_synthesis_ms",
]
...
def _run(cmd: list[str], env: dict) -> None:
    print(f"$ {' '.join(cmd)}  (env override: { {k: v for k, v in env.items() if k in (HOP_ENV, RERANK_ENV, 'RAG_BENCHMARK_TIMESTAMP')} })")
...
def _sweep_point_hop(value: str, corpus_tag: str, queries_file: str, model: str, timestamp: str) -> dict:
    ...
    _run([PYTHON, "main.py", "--mode", "hop_rebuild", "--strategy", "prehop",
          "--corpus-tag", corpus_tag], env)
    ...
    _run([PYTHON, "main.py", "--mode", "benchmark", "--strategy", "prehop",
          "--corpus-tag", corpus_tag, "--queries_file", queries_file,
          "--model", model], env)
...
def _sweep_point_rerank(value: str, corpus_tag: str, queries_file: str, model: str, timestamp: str) -> dict:
    env = {RERANK_ENV: value, "RAG_BENCHMARK_TIMESTAMP": timestamp}
    _run([PYTHON, "main.py", "--mode", "benchmark", "--strategy", "prehop",
          "--corpus-tag", corpus_tag, "--queries_file", queries_file,
          "--model", model], env)
    ...
    result_path = ROOT / "data" / "results" / timestamp / "prehop" / corpus_tag / f"prehop_{corpus_tag}.json"
    summary = json.loads(result_path.read_text()) if result_path.exists() else {}
    return {"value": value, "graph_stats": {}, "summary": summary}


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--param", choices=["hop", "rerank"], required=True)
    ...
    points = []
    for value in values:
        timestamp = f"sweep_{args.param}_{_slug(value)}"
        if args.param == "hop":
            point = _sweep_point_hop(value, args.corpus_tag, args.queries_file, args.model, timestamp)
        else:
            point = _sweep_point_rerank(value, args.corpus_tag, args.queries_file, args.model, timestamp)
        points.append(point)

    (out_dir / f"{args.param}_sweep.json").write_text(json.dumps(points, indent=2), encoding="utf-8")

    with (out_dir / f"{args.param}_sweep.csv").open("w", newline="") as fh:
        w = csv.writer(fh)
    ...
    print(f"\nSweep complete -> {out_dir}/{args.param}_sweep.{{json,csv}}")
    for point in points:
        s = point.get("summary") or {}
        print(f"  {args.param}={point['value']}: judge={s.get('avg_llm_judge_score', 'n/a')} "
