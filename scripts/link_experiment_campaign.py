...
def ready_jobs(jobs,states,active,*,max_active=2,strategy_filter=None):
    ...
    if strategy_filter:
        ready=[j for j in ready if j.get('strategy')==strategy_filter or strategy_filter in j['id']]
...
def run(plan_path, *, resume=False):
    ...
    if resume:
        states.update(json.loads((base/'status.json').read_text())['tasks'])
    for job in jobs:
        if resume:
            if states[job['id']]['state']=='running':
                job['adopt']=states[job['id']]['identity'];active.append(job)
        elif job.get('adopt'):
    ...
    while True:
        ...
        for job in ready_jobs(jobs,states,active,max_active=plan.get('max_active',2),strategy_filter=plan.get('execution_filter')):
            ...
            handle=(base/(job['id']+'.log')).open('a' if resume else 'x')
...
def main():
    ...
    r=sub.add_parser('run');r.add_argument('plan',type=Path);r.add_argument('--resume',action='store_true')
    ...
    a=p.parse_args()
    if a.mode=='run':run(a.plan,resume=a.resume)
